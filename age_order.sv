...
(
    ...
    input  logic               alloc_en,   // a new slot enabled (allocated)
    input  logic [N-1:0]       alloc_oh,   // one-hot: which free slot the NEW tran goes to
    input  logic               del_en,     // an existing tran finished and its slot is being freed
    input  logic [N-1:0]       del_oh,     // one-hot: which old slot is being DELETED (freed)
